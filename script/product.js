  ...
  try {
    ...
    if (allProducts) {
      try {
        ...
        allProducts = setDefaultProducts();
      }
    } else {
      allProducts = setDefaultProducts();
    }
    ...
    function setDefaultProducts() {
      let defaultProducts = [
        ...
        },
        {
          ...
          details:
            "salicylic acid, benzoyl peroxide, tea tree oil, ferulic acid",
          description:
        ...
        },
        {
          ...
          details:
            "vitamin C, charcoal, ceramides, hyaluronic acid, niacinamide",
          description:
    ...
    let container = document.querySelector("[all-content]");
    ...
    function fetchData() {
      ...
      products.forEach((product) => {
        container.innerHTML += `
            <div class="card product-card col-4 m-auto p-0">
                <img src="${
                  product.img_url
                ...
                <div class="card-body">
                    ...
                    <div class="product-btns-box">
                        <a class="btn product-btn" data-product-id="${parseInt(
                          product.id
                        )}" data-bs-toggle="modal" data-bs-target="#detailModal">See More</a>
                        <a class="btn product-btn" cart-btn data-add-to-cart-id="${product.id}>Add to Cart</a>
                    </div>
                </div>
            </div>
        `;
      ...
      document
        .querySelectorAll(".product-btn[data-product-id]")
        .forEach((button) => {
    ...
    let displayProductDetails = (id) => {
      try {
        let product = allProducts.find((p) => p.id === Number(id));
        id = Number(id);
        console.log("Displaying product details for:", id);
        console.log(product);

        if (product) {
          document.getElementById("productModalLabel").textContent =
            product.productName;
          document.getElementById("productModalImg").src = product.img_url;
          document.getElementById(
            "productModalPrice"
          ).textContent = `R${product.price}`;
          document.getElementById("productModalVolume").textContent =
            product.volume;
        ...
        // Update modal title and body with product details
    ...
    container.innerText = "Please contact the administrator";
  }
}

// const cartAddBtn = document.querySelectorAll("cart-btn");

// function pushCart() {
//   cartAddBtn.addEventListener("click", () => {
//     let log = console.log("cart will show up here");
//     checkoutProducts.push(log);
//   });
// }
